...
from gpytorch.kernels import RBFKernel, ScaleKernel, MaternKernel, PolynomialKernel, RQKernel, SpectralMixtureKernel, \
    ProductStructureKernel, GridInterpolationKernel, SpectralDeltaKernel, LinearKernel, RFFKernel
...
class ToyDeepGPHiddenLayer(DeepGPLayer):
    def __init__(self, input_dims, output_dims, num_inducing=128, mean_type='constant'):
        ...
        self.covar_module = RFFKernel(batch_shape=batch_shape, num_samples=10)
...
class DeepGP(DeepGP):
    ...
    def forward(self, inputs):
        ...
        output = self.last_layer(hidden_rep2,inputs)
        return output
...
time12 = time.strftime("%Y-%m-%d %H:%M:%S",time.localtime())
file_name = 'DGP_ScaleRBF_3layer_short_Cholesky_Epoch20'
