...
#dataframe = pd.read_csv('E:\CjlFile\组会文件\深度高斯过程\DGP\\3.4集成异核高斯模型\\3.4集成异核高斯模型\data\\6000.csv',header=None)
dataframe = pd.read_csv('E:\CjlFile\组会文件\深度高斯过程\DGP\\3.4集成异核高斯模型\\3.4集成异核高斯模型\data\dianji_cjl.CSV',header=None)
#dataframe = pd.read_csv('E:\CjlFile\组会文件\深度高斯过程\DGP\\3.4集成异核高斯模型\\3.4集成异核高斯模型\data\PM2.5.csv',header=None)
...
class DeepGP(DeepGP):
    def __init__(self, train_x_shape):
        ...
        hidden_layer2 = ToyDeepGPHiddenLayer(
            input_dims=hidden_layer1.output_dims+train_x_shape[-1],
            output_dims=num_output_dims,
            mean_type='constant',
        )
        ...
        hidden_layer4 = ToyDeepGPHiddenLayer(
            input_dims=hidden_layer3.output_dims+train_x_shape[-1],
            output_dims=num_output_dims,
            mean_type='constant',
        )
        ...
        hidden_layer6 = ToyDeepGPHiddenLayer(
            input_dims=hidden_layer5.output_dims+train_x_shape[-1],
            output_dims=num_output_dims,
            mean_type='constant',
        )
    ...
    def forward(self, inputs):
        ...
        hidden_rep2 = self.hidden_layer2(hidden_rep1,inputs)
        hidden_rep3 = self.hidden_layer3(hidden_rep2)
        hidden_rep4 = self.hidden_layer4(hidden_rep3,inputs)
        hidden_rep5 = self.hidden_layer5(hidden_rep4)
        hidden_rep6 = self.hidden_layer6(hidden_rep5,inputs)
        hidden_rep7 = self.hidden_layer7(hidden_rep6)
        output = self.last_layer(hidden_rep7)
        return output
...
time12 = time.strftime("%Y-%m-%d %H:%M:%S",time.localtime())
file_name = 'DGP_ScaleRBF_8layer_Cholesky_short_Epoch80_100inducing'
