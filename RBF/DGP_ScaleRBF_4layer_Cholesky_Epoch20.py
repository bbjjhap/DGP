...
#dataframe = pd.read_csv('E:\CjlFile\组会文件\深度高斯过程\DGP\\3.4集成异核高斯模型\\3.4集成异核高斯模型\data\InverterDataSet.csv',header=None)
...
time12 = time.strftime("%Y-%m-%d %H:%M:%S",time.localtime())
file_name = 'DGP_ScaleRBF_4layer_Cholesky_Epoch20'
...
txt_file.write('NLL :'+str(-test_lls.mean().item()))
txt_file.write('        \n')
txt_file.close()
#

wb = openpyxl.load_workbook(
    "E:\CjlFile\组会文件\深度高斯过程\DGP\\3.4集成异核高斯模型\\3.4集成异核高斯模型\code\\test\dgp_model\\result\short-dgp.xlsx")
sheet = wb.active
for i in range(0,len(predictive_means.mean(0))):
    #if sheet.cell(row=i, column=1).value != None:
     sheet.cell(row=i+1, column=1).value = str(predictive_means.mean(0)[i].numpy())
     # sheet.cell(row=i + 1, column=2).value = str(test_y[i]).replace('[','').replace(']','')
     sheet.cell(row=i + 1, column=2).value = str(test_y[i].numpy()).replace('[','').replace(']','')
wb.save("E:\CjlFile\组会文件\深度高斯过程\DGP\\3.4集成异核高斯模型\\3.4集成异核高斯模型\code\\test\dgp_model\\result\short-dgp.xlsx")
