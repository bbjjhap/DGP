...
dataframe = pd.read_csv('D:\\new dgp\DGP\DGP\data\housing3.csv',header=None)
...
class DeepGP(DeepGP):
    ...
    def predict(self, test_loader):
        ...
        return torch.cat(mus, dim=0), torch.cat(variances, dim=0), torch.cat(lls, dim=0)
...
# 可视化所有样本的SHAP值摘要
shap.summary_plot(shap_values, test_x, feature_names=["Feature1", "Feature2", ...])
...
# 创建SHAP解释器
explainer = shap.DeepExplainer(lambda x: model(x)[0], train_x)
...
# time12 = time.strftime("%Y-%m-%d %H:%M:%S",time.localtime())
# file_name = 'DGP_ScaleRBF_3layer_Delta_Gamma_Epoch20'
